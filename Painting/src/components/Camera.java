...
public class Camera
{
	...
	public Camera(Point location, Point focus, Point up, double zoom,
	...
	private void createCoordinateSystem()
	{
		...
		k = k.normalize();// Only normalized vectors are necessary for an orthogonal curvilinear coordinate system.
		...
		j = j.sub(k.multScalar(j.dot(k)));// Subtract the part of J parallel to k from j to get the perpendicular part.
	...
	public Point getDirection(double x, double y)
	{
		...
		Point dz = i.multScalar(x/factor).add(j.multScalar(y/factor)).add(k.multScalar(zoom));
