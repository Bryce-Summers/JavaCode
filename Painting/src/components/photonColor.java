...
public class photonColor
{
	...
	public photonColor(int hex)
	{
		this.red   = Colors.rgbToRed(hex)   / 255.0;
		this.green = Colors.rgbToGreen(hex) / 255.0;
		this.blue  = Colors.rgbToBlue(hex)  / 255.0;
	}
	...
	public static photonColor lerp(photonColor start, photonColor end, double time)
	{
		double r = lerp(start.red,   end.red,   time);
		double g = lerp(start.green, end.green, time);
		double b = lerp(start.blue,  end.blue,  time);
		
		return new photonColor(r, g, b);

	}
	
	public static double lerp(double small, double large, double time)
	{
		double pSmall = 1.0 - time;
		double pLarge = time;
		
		return small*pSmall + large*pLarge;
	}
