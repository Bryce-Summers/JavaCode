...
public class ccTrainCarIcon extends Image_vectorGeometry
{

	final int hue, sat;
	final int val;
	
	public ccTrainCarIcon(Dimension dim, int hue, int sat)
	{
	...
	public ccTrainCarIcon(int w, int h, int hue, int sat)
	{
		super(w, h, true);
	...
	public ccTrainCarIcon(int w, int h, int hue, int sat, int val)
	{
		super(w, h, true);
	...
	public void i_geoms()
	{
		...
		wheel(v(thickness*6, h - thickness*5), radius);
		wheel(v(w - thickness*6, h - thickness*5), radius);
	}
	
	void wheel(Vector v, double radius)
	{
