...
import static BryceMath.Calculations.MathB.sqrt;
...
public class ccHeart extends ColorCalculator
{
	...
	public Color getColor(double x, double y)
	{
		...
		{
			//return Color_hsv(0,100,c/3.63*100/coef);
			//return Color_hsv(0, 100, 100);
			
			// Smoothly interpolate a red outer outline.
			return Color_hsv(0, 100, 100 * Math.pow(sin((c - bound1)/(bound2 - bound1)*Math.PI), .3));
		}
	...
	{
		//return 100*f(x / 100);
		return 100*Math.pow(f(x/100), .07);
		//return 100*Math.pow(f(x), .3);
		//return 100*Math.pow(f(x), .1);
	}
